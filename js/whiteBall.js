function WhiteBall(){
  //view section;
	...
	this.velocityStarter = function(x,y,speedValue){
		var cosTheta = (that.posX-x)/Math.sqrt((Math.pow(that.posX-x,2)+Math.pow(that.posY-y,2)));
		var sinTheta = (that.posY-y)/Math.sqrt((Math.pow(that.posX-x,2)+Math.pow(that.posY-y,2)));
